...
namespace Bililive_dm
{
    public class FontSizeValidationRule : ValidationRule
    {
        public override ValidationResult Validate(object value, CultureInfo cultureInfo)
        {
            ...
            if (str == null) return new ValidationResult(false, "不可为空");
            float ftvalue;
            if (!float.TryParse(str, out ftvalue)) return new ValidationResult(false, "不是数字");
            if (ftvalue < 0) return new ValidationResult(false, "必须是正数");
